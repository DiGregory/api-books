...
async def create_book(
    book: IncomingBook, session: DBSession, curr_user=Depends(get_current_user)
):  # прописываем модель валидирующую входные данные и сессию как зависимость.
    # это - бизнес логика. Обрабатываем данные, сохраняем, преобразуем и т.д.
    new_book = Book(
        title=book.title, author=book.author, seller_id=book.seller_id, year=book.year, count_pages=book.count_pages
    )
...
async def update_book(book_id: int, new_data: ReturnedBook, session: DBSession, curr_user=Depends(get_current_user)):
