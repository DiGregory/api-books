...
async def test_create_book(db_session, async_client):
    ...
    seller = Seller(first_name="John", last_name="Doe", email="john.doe@example.com", password="password123")
    db_session.add(seller)  # Добавляем экземпляр Seller, а не словарь
    await db_session.commit()
    seller_id = seller.id  # Получаем ID созданного продавца

    # Получаем JWT токен для аутентифицированного продавца
    response = await async_client.post(
        "/api/v1/token/", data={"username": "john.doe@example.com", "password": "password123"}
    )
    token = response.json()["access_token"]

    # Теперь создаем книгу, указывая ID продавца
    data = {
        "title": "Wrong Code",
        "author": "Robert Martin",
        "pages": 104,
        "year": 2007,
        "seller_id": seller_id,  # Указываем ID продавца
    }
    response = await async_client.post("/api/v1/books/", headers={"Authorization": f"Bearer {token}"}, json=data)
    assert response.status_code == status.HTTP_201_CREATED

    result_data = response.json()

    # Обновите утверждение, чтобы включить 'seller_id' в проверку
    assert result_data["title"] == "Wrong Code"
    assert result_data["author"] == "Robert Martin"
    assert result_data["count_pages"] == 104
    assert result_data["year"] == 2007
